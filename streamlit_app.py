...
if st.session_state.step == 1:
    ...
    with st.form("interview_form"):
        ...
        if user_type == 'jugendlich':
            ...
            for i in range(anzahl_schulen):
                ...
                col1, col2 = st.columns(2)
                with col1:
                    schule_name = st.text_input("Name der Schule " + str(i+1) + " *", key="schule_name_" + str(i))
                    von = st.date_input("Von (Schule " + str(i+1) + ") *", key="von_" + str(i), value=date(2015, 8, 1))
                with col2:
                    schule_ort = st.text_input("Ort (Schule " + str(i+1) + ")", key="schule_ort_" + str(i))
                    bis_noch = st.checkbox("Besuche ich noch", key="noch_" + str(i))
                    if not bis_noch:
                        bis = st.date_input("Bis (Schule " + str(i+1) + ") *", key="bis_" + str(i), value=date.today())
                    else:
                        bis = "heute"
                abschluss = st.text_input("Abschluss/Klasse (Schule " + str(i+1) + ")", key="abschluss_" + str(i))
                schulen_temp.append({'name': schule_name, 'ort': schule_ort, 'von': von, 'bis': bis, 'abschluss': abschluss})
            st.markdown("---")
            ...
            for i in range(anzahl_geschwister):
                ...
                with col1:
                    g_vorname = st.text_input("Vorname *", key="g_vorname_" + str(i))
                with col2:
                    g_alter = st.number_input("Alter *", min_value=0, max_value=100, value=10, key="g_alter_" + str(i))
                with col3:
                    g_beruf = st.text_input("Beruf/Schule (optional)", key="g_beruf_" + str(i))
                geschwister_temp.append({'vorname': g_vorname, 'alter': g_alter, 'beruf': g_beruf})
        else:
        ...
        if submitted:
            if not vorname or not nachname or not staerken or not ziele:
                ...
                if user_type == 'jugendlich':
                    ...
                    st.session_state.user_data.update({'vorname': vorname, 'nachname': nachname, 'geburtsdatum': geburtsdatum.strftime('%d.%m.%Y'), 'alter': alter_berechnet, 'hobbies': hobbies, 'staerken': staerken, 'ziele': ziele, 'schulen': schulen_temp, 'vater_name': vater_name, 'vater_beruf': vater_beruf, 'mutter_name': mutter_name, 'mutter_beruf': mutter_beruf, 'geschwister': geschwister_temp})
                else:
    ...
    if st.session_state.get('analysis_done'):
        ...
        for i, comp in enumerate(competencies, 1):
            st.markdown("**" + str(i) + ". " + comp['name'] + "**")
            st.write(comp['description'])
            st.progress(comp['strength'] / 100)
            st.markdown("---")
        st.subheader("Passende Zitate:")
        selected = st.radio("Wähle dein Zitat:", options=range(len(quotes)), format_func=lambda i: '"' + quotes[i]["text"] + '" - ' + quotes[i]["author"])
    ...
    user_data = st.session_state.user_data
    colors_list = user_data.get('colors', ['#3498db', '#2ecc71'])
    quote = user_data.get('selected_quote', {})
    st.markdown('<div style="background:linear-gradient(135deg,' + colors_list[0] + ',' + colors_list[1] + ');padding:60px;border-radius:15px;color:white;text-align:center"><h1>Bewerbungsdossier</h1><h2>' + user_data["vorname"] + ' ' + user_data["nachname"] + '</h2><p style="font-style:italic">"' + quote.get("text", "") + '"</p><p>— ' + quote.get("author", "") + '</p></div>', unsafe_allow_html=True)
    if st.button("PDF erstellen", type="primary", use_container_width=True):
        with st.spinner("Erstelle PDF..."):
            try:
                ...
                story.append(Paragraph(user_data['vorname'] + " " + user_data['nachname'], styles['Heading1']))
                if user_data.get('type') == 'jugendlich':
                    story.append(Paragraph("Geburtsdatum: " + user_data.get('geburtsdatum', ''), styles['Normal']))
                story.append(Spacer(1, 0.3*inch))
                story.append(Paragraph('"' + quote.get("text",
